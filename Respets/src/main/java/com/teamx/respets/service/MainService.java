...
public class MainService {
	...
	public ModelAndView index(HttpServletRequest request) {
		...
		if(request.getSession().getAttribute("no")==null
				|| request.getSession().getAttribute("no").toString().substring(0, 1).equals("P")) {
			aboList = mDao.selectPerBoardList();
		} else if(request.getSession().getAttribute("no").toString().substring(0, 1).equals("B")) {
			aboList = mDao.selectBusBoardList();
		} else {
			aboList = mDao.selectBoardList();
		}
